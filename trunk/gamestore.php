...
$categories = $db->query_read("SELECT catid, name FROM ".TABLE_PREFIX."gamecategory WHERE visible = 1 ORDER BY `order`");
while($category = $db->fetch_array($categories)){
	$games = $db->query_read("SELECT gid, name, icon FROM ".TABLE_PREFIX."game WHERE catid = {$category['catid']} AND visible = 1 ORDER BY RAND() LIMIT 0, 6");
	$gamelist = '';
	while($game = $db->fetch_array($games)){
		$gamelist .= <<<eot
<td>
	<a href="game.php?gid={$game['gid']}"><img src="{$game['icon']}" /></a>
	<p><a href="game.php?gid={$game['gid']}">{$game['name']}</a></p>
</td>
eot;
	}
	$forumbits .= <<<eot
...
<div class="forumbit_nopost L1">
	<div class="forumhead foruminfo L1 collapse">
		<h2>
			<span class="forumtitle">
				<a href="gamecategory.php?catid={$category['catid']}">Game {$category['name']}</a>
			</span>
		</h2>
	</div>
	...
	<div class="forumbit_nopost L2">
		<div class="forumrow">
			...
			<div class="readmore"><a href="gamecategory.php?catid={$category['catid']}">Xem thêm &gt;&gt;</a></div>
			<div class="clear"></div>
		</div>
	</div>
